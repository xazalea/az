...
export class Parser {
    ...
    private parseStatement(): Statement {
        ...
        try {
            ...
            // Flexible Grammar Fallback: Treat parsing errors as implicit AI instructions
            // AI That Interprets Ambiguous Code (Task 10 continued refinement)
            this.current = start;
            
            // Check if we are inside a block structure or indentation seems wrong?
            // For now, consume line until newline
            let instruction = "";
            let hasContent = false;
            
            while (!this.check(TokenType.NEWLINE) && !this.check(TokenType.EOF)) {
                // If we hit a DEDENT or INDENT in the middle, it's tricky. 
                // But usually those are line-start.
                // Just consume everything.
                if (this.check(TokenType.DEDENT)) break;
                
                instruction += this.peek().value + " ";
                this.advance();
                hasContent = true;
            }
            
            if (hasContent && instruction.trim().length > 0) {
                return {
                    type: 'AIStatement',
    ...
    private parseAI(): AIStatement {
        // AI as an Execution Unit (“AI Operators”)
        // Supports explicit `ai "instruction"` syntax
        let instruction: string | undefined;
        if (this.match(TokenType.STRING)) {
            instruction = this.previous().value;
        }
        ...
        if (!instruction && !body) {
            throw new Error("AI statement requires an instruction string or a block.");
        }
